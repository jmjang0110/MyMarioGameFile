...
class CMonsterManager():
    ...
    def create_Monster_Stage3(self):
        ...
        for i in range(ROW - 1):
            for j in range(COLUM - 1):
                ...
                if CMonsterManager.MapData[i][j] == 1:
                    ...
                    if Monsterindex == 1:
                        monster = Monster1()
                        monster.setSpot(MapTileManager.mapTile_Data_3[i][j].x,
                                        MapTileManager.mapTile_Data_3[i][j].y + 10,
                                        MapTileManager.mapTile_Data_3[i][j].Tile_Width_size // 2,
                                        MapTileManager.mapTile_Data_3[i][j].Tile_Width_size // 2)
                    elif Monsterindex == 2:
                        monster = Monster2()
                        monster.setSpot(MapTileManager.mapTile_Data_3[i][j].x,
                                        MapTileManager.mapTile_Data_3[i][j].y + 40,
                                        MapTileManager.mapTile_Data_3[i][j].Tile_Width_size // 2,
                                        MapTileManager.mapTile_Data_3[i][j].Tile_Width_size // 2)
                    elif Monsterindex == 3:
                        monster = Monster3()
                        monster.setSpot(MapTileManager.mapTile_Data_3[i][j].x,
                                        MapTileManager.mapTile_Data_3[i][j].y + 20,
                                        MapTileManager.mapTile_Data_3[i][j].Tile_Width_size // 2,
                                        MapTileManager.mapTile_Data_3[i][j].Tile_Width_size // 2)
                    ...
                    monster = Monster6()
                    monster.setSpot(MapTileManager.mapTile_Data_3[i][j].x - 40,
                                    MapTileManager.mapTile_Data_3[i][j].y - 30, 0, 0)
                    CMonsterManager.MonsterData.append(monster)
                    game_world.add_object(monster, 1)
